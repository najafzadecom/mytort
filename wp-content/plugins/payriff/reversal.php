...
if (isset($_POST['refund_amount']) && $_POST['refund_amount'] && is_user_logged_in()) {
	...
	$sessionId = $order_meta['order_session_id'][0];
	...
	if (!empty($orderId) && !empty($sessionId)) {
		...
		// Adding order note.
		if ($result['status']) {
			$note = __('Success! The payment was refunded.', 'payriff');
		}else{
			$note = __('Something wrong. The payment wasn\'t refunded. Please, try later.', 'payriff');
		}
